...
class ChatHandler @Inject constructor(
    ...
    fun initHandler() {
        ...
        listenerDoc1 = messageCollectionGroup.whereArrayContains(FireStoreCollection.CHAT_USER, fromUser!!)
            .addSnapshotListener { snapShots, error ->
                if (error != null || snapShots == null || snapShots.metadata.isFromCache) {
                    //Log.e(TAG.ERROR, "${snapShots?.metadata?.isFromCache}")
    ...
    private fun onFetchDocuments() {
        messageCollectionGroup.whereArrayContains(FireStoreCollection.CHAT_USER, fromUser!!).get()
